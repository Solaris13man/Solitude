...
export function deal(seed: number): GameState {
  ...
  const cells: Card[][] = Array.from({ length: 8 }, () => []);
  // 48 cards into 8 columns of 6, all face up.
  for (let i = 0; i < 48; i++) {
    tableau[i % 8]!.push({ ...deck[i]!, faceUp: true });
  }
  // Remaining 4 cards seed the first four cells.
  for (let i = 0; i < 4; i++) {
    cells[i]!.push({ ...deck[48 + i]!, faceUp: true });
  }
  return {
    game: 'eightoff',
    seed,
...
/** Eight Off tableau builds down *in suit*; only a King leads an empty column. */
function fitsOnCascade(card: Card, target: Card | undefined): boolean {
  if (!target) return card.rank === 13; // only a King may lead an empty column
...
export function canPickRun(state: GameState, ref: PileRef, depth: number): boolean {
  ...
  // Besides columns, only cell cards can be picked up; foundation cards stay put.
...
export function applyMove(state: GameState, move: Move): void {
  if (move.type !== 'move') throw new Error('eightoff has no stock');
...
/**
 * Auto-finish is safe once every column is ordered by descending rank from
 * bottom to top: the lowest card needed is then always exposed, and cells are
 * always accessible.
 */
...
/** Tap: foundation first, then a column, then (single cards) a free cell. */
...
/**
 * Hint priority: foundation plays, column builds (preferring runs and moves
...
export function findHint(state: GameState): Move | null {
  ...
  // Column moves: prefer ones that land on a card (building) over empties.
...
export const eightOffRules: Ruleset = {
  id: 'eightoff',
  name: 'Eight Off',
  tableauCount: 8,
  foundationCount: 4,
